 ...
 */
 contract AuctionRepository{
    ...
    //Mapping from owner to alist of owned auctions
    mapping(address => uint[]) public auctionOwner;

    // Bid struc to hold bidder and amount 
    struct Bid {
        address payable from; 
        uint256 amount; 
    }

    //Auction struc which holds all the required info 
    struct Auction{
        string name; 
    ...
    * reminder: view function promise not to modify the state
    */
    ...
    */
    function getCurrentBid(uint _auctionId) public view returns(uint256, address){
        ...
        //if there arte bids refund the last bid
        if( bidsLength > 0 ) {
